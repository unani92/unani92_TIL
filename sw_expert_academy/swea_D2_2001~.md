...
while row > 0 and col > 0 : 
    ...
    for i range(offset + 1, offset + col) : # 위 -> 아래
        cnt += 1
        matrix[i][offset+col-1] = cnt   # n-1개 숫자가 차례로 채워짐
        
    for i in range(offset+row-2,offset-1,-1) :   # 오 -> 왼
        cnt += 1
        matrix[offset+rowl-1][i] = cnt    # n-1개 채워짐
        
    for i in range(offset + col -2, offset, -1) :   # 아래 -> 위
        cnt += 1
        matrix[i][offset] = cnt       # n-2개 채워짐
'''     
한 사이클을 돌고 나면 파라미터를 조정해 준다. 그림을 봐도 알 수 있듯이 한 사이클을 돌면 양 옆과 위 아래가 채워지고, 다음 사이클에서 채워야 할 부분은 n-2개이기 때문에 행과 열은 2개씩 빼주어야 한다. 
'''
	offset += 1
    row -= 2
    col -= 2
